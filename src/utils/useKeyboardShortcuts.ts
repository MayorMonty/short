...
export function onKeydown<T>(options: Options): (event: React.KeyboardEvent<T> | KeyboardEvent) => void {
    return (event: React.KeyboardEvent<T> | KeyboardEvent) => {

        const nativeEvent = event instanceof KeyboardEvent ? event : event.nativeEvent;
        ...
        if (typeof options.key === "string") {
            ...
            options.callback(nativeEvent);

        } else if (typeof options.key === "function") {
            if (!options.key(nativeEvent)) {
                return;
            }
